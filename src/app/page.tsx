...
export default function Home() {
  ...
  const handleSubmitRegister = async (formData: any) => {
    try {
      ...
      const result = await userRepository.addUser({ email, password, username } as UserEntity, dispatch);
      if (result) {
        router.push("/panel");
      }
    } catch (err: any) {
  ...
  const handleSubmitLogin = async (formData: any) => {
    try {
      ...
      const result = await userRepository.loginUser({ email, password } as UserEntity, dispatch);
      if (result) {
        router.push("/panel");
      }
    } catch (err: any) {
